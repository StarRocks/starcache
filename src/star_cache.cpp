...
Status StarCache::_read_cache_item(const CacheId& cache_id, CacheItemPtr cache_item, off_t offset, size_t size,
                                   IOBuf* buf) {
    buf->clear();
...
size_t StarCache::_continuous_segments_size(const std::vector<BlockSegmentPtr>& segments) {
    size_t size = 0;
    for (auto& seg : segments) {
        if (seg->offset != size) {
            break;
...
Status StarCache::_flush_block_segments(CacheItemPtr cache_item, const BlockKey& block_key,
...
void StarCache::_process_evicted_disk_items(const std::vector<CacheId>& evicted) {
